...
export class CartStateService {
    ...
    private add(state: Signal<State>, productItem: ProductItemCart) {
        const isInCart = state().products.find(
            productInCart => productInCart.product.id === productItem.product.id
        )
        if(!isInCart) {
            return {
                products: [...state().products, { ...productItem, quantity: 1}]
            }
        }
        isInCart.quantity += 1
